...
draw_set_font(lang_font(fnt_mars));
...
if (TURN == BATTLE_TURNS.PLAYER && page.current == BATTLE_PAGES.MAIN && READY) {	
	draw_each_letter(txt.x,txt.y,flavor_text,,,,,,,,obj_bulletBorder.bbox_right-txt.x,true,flavor_writer)
...
switch(TURN) {
	case BATTLE_TURNS.PLAYER:
		switch(page.current) {
			...
			case BATTLE_PAGES.ACT_CHOOSE:
				var lr = (InputPressed(INPUT_VERB.RIGHT)-InputPressed(INPUT_VERB.LEFT));
				if (lr != 0) {
				...
				for(var i = 0; i < array_length(target_enemy.instance.action); i++) {
					...
					var __y = txt.y+16*(i mod row_length), __x = (txt.x+heart_offset.space)+125*(i div row_length)
			...
			case BATTLE_PAGES.MERCY:
				for(var i = 0; i < array_length(mercy_actions); i++) {
					var __y = txt.y+18*i, __x = txt.x+heart_offset.space, c = TEXTconfig.color
	...
	case BATTLE_TURNS.END:
		if (box_is_normal()) {
			draw_each_letter(txt.x,txt.y,$"* YOU WON!<w,10>\nYou earned {lootbox.xp} XP and {lootbox.gold} GOLD",,,,,,,,obj_bulletBorder.bbox_right-txt.x,true,win_writer)
		}
	break;	
}
